...
const DashboardHeader = () => {
  ...
  const handleFaucetClick = () => {
    const faucetUrl = import.meta.env.VITE_MONAD_FAUCET_URL || 'https://faucet.monad.xyz/';
    ...
    try {
      ...
      if (error.message && error.message.includes('nativeCurrency.symbol does not match')) {
        toast.info('Network Already Added', {
          description: 'The Monad Testnet is already in your wallet. Please switch to it manually.'
        });
      } else {
        toast.error(
      ...
      <div className="relative z-20 bg-charcoal-900/80 backdrop-blur-xl border-b border-golden-border shadow-charcoal-deep">
        ...
        <div className="flex items-center justify-between py-3 md:py-6 px-4 md:px-8">
          ...
          >
            ...
            <div>
              <h1 className="text-2xl md:text-4xl font-playfair font-bold golden-text">MonadAI</h1>
              <p className="text-xs font-inter text-gold-300/60 mt-0.5 md:mt-1 hidden sm:block">AI-powered DeFi navigator for Monad</p>
            </div>
          </div>
