...
import { Send, Bot, User, BarChart2, ArrowRight, TrendingUp, Search, Shield, Mic, MicOff, Globe } from 'lucide-react';
...
const CHAT_STORAGE_KEY = 'monadai_chat_messages';
const USER_PROFILE_KEY = 'monadai_user_profile';
const CONVERSATION_CONTEXT_KEY = 'monadai_conversation_context';

// Enhanced market intelligence data for AI suggestions with realistic allocation changes
const marketInsights = [
  {
    type: 'trending',
    content: "Based on on-chain data analysis, MON (Layer 1) is showing a significant increase in whale accumulation. Three addresses have accumulated over $2.7M in the last 48 hours. Consider increasing your Layer 1 allocation by 5%.",
    action: {
      type: 'rebalance',
      description: 'Increase Layer 1 allocation based on whale accumulation data',
      changes: [
  ...
  {
    type: 'volume',
    content: "PINGU is experiencing abnormal trading volume, up 320% in the last 24 hours. Social sentiment analysis shows this meme coin trending across major platforms. Consider a small speculative position of 2%.",
    action: {
      type: 'trade',
  ...
  {
    type: 'news',
    content: "Breaking: Crystal Exchange volume has increased 78% following the MON price rally. According to our technical analysis, the sMON token is currently undervalued based on TVL metrics. Consider increasing your DeFi exposure.",
    action: {
      type: 'rebalance',
  ...
  {
    type: 'risk',
    content: "Risk assessment alert: Your portfolio exposure to meme tokens (22%) exceeds recommended thresholds. Consider rebalancing to reduce volatility, particularly with CHOG token showing signs of distribution by early investors.",
    action: {
      type: 'protection',
  ...
  {
    type: 'defi',
    content: "DeFi yield opportunities analysis: aprMON protocol has increased lending yields to 8.2% APY for stablecoins. Consider shifting 3% from your Layer 1 allocation to DeFi to capitalize on this yield opportunity.",
    action: {
      type: 'rebalance',
  ...
  {
    type: 'layer1',
    content: "On-chain metrics for MON are showing strong growth with daily active addresses up 34% this month. The upcoming protocol upgrade could drive further adoption. Consider increasing your Layer 1 exposure.",
    action: {
      type: 'rebalance',
  ...
  {
    type: 'rwa',
    content: "Real World Assets (RWA) tokens are showing increased institutional adoption. Traditional asset tokenization platforms have onboarded three new institutional investors. Consider increasing your stablecoin allocation for portfolio stability.",
    action: {
      type: 'rebalance',
  ...
  {
    type: 'security',
    content: "Security analysis of your portfolio indicates high exposure to newer, less audited protocols. Consider shifting 5% from meme tokens to more established projects with stronger security track records.",
    action: {
      type: 'protection',
  ...
  {
    type: 'yield',
    content: "Yield analysis shows MON staking returns have increased to 9.3% APY following the latest protocol upgrade. Consider increasing your Layer 1 allocation to capture these enhanced staking rewards.",
    action: {
      type: 'rebalance',
  ...
  {
    type: 'innovation',
    content: "The DeFi token sector is experiencing accelerated innovation with shMON launching new shared staking mechanisms. Early adopters could benefit from significant growth. Consider increasing your DeFi allocation.",
    action: {
      type: 'rebalance',
...
const formatPortfolioSuggestion = (action: any, content: string): string => {
  ...
  // Add summary
  const totalIncrease = increases.reduce((sum: number, c: any) => sum + (c.to - c.from), 0);
  const totalDecrease = decreases.reduce((sum: number, c: any) => sum + (c.from - c.to), 0);
  
  formattedContent += '📋 **Summary:**\n';
  formattedContent += `   💰 Total Reallocation: ${Math.max(totalIncrease, totalDecrease)}%\n`;
  formattedContent += `   🎯 Strategy: ${action.type === 'rebalance' ? 'Portfolio Rebalancing' : 
                                          action.type === 'protection' ? 'Risk Mitigation' : 
                                          action.type === 'trade' ? 'Trading Opportunity' : 'Portfolio Optimization'}\n`;
...
const getDescriptionForChange = (change: any, type: 'increase' | 'decrease'): string => {
  const descriptions: { [key: string]: { increase: string, decrease: string } } = {
    'stablecoin': {
      increase: 'เพิ่มความมั่นคงของพอร์ตด้วยการถือครอง Stablecoins มากขึ้น',
      decrease: 'ลดการถือครอง Stablecoins เพื่อเพิ่มผลตอบแทน'
    },
    'bigcap': {
      increase: 'เพิ่มการลงทุนในสินทรัพย์ที่มีมูลค่าตลาดสูงซึ่งมีความผันผวนน้อยกว่า',
    ...
    'l1': {
      increase: 'เพิ่มการลงทุนใน Layer 1 protocols เพื่อรองรับการเติบโตของ ecosystem',
      decrease: 'ลดการถือครอง Layer 1 เพื่อปรับสมดุลพอร์ต'
    },
    'ai': {
      increase: 'เพิ่มการลงทุนใน AI tokens เพื่อโอกาสในการเติบโตจากเทคโนโลยี AI',
      decrease: 'ลดการลงทุนใน AI tokens เพื่อลดความเสี่ยงจากความผันผวน'
    },
    'rwa': {
      increase: 'เพิ่มการลงทุนใน Real World Assets เพื่อความมั่นคงและการกระจายความเสี่ยง',
      decrease: 'ลดการถือครอง RWA เพื่อเพิ่มการลงทุนในสินทรัพย์ที่เติบโตเร็วกว่า'
    }
  };
...
const AIChat = () => {
  ...
  const [speechLanguage, setSpeechLanguage] = useState<'th-TH' | 'en-US'>('en-US');
        ...
        recognitionInstance.onerror = (event: SpeechRecognitionErrorEvent) => {
          ...
          if (import.meta.env.VITE_OPENAI_API_KEY && !useWhisperFallback) {
            toast({
              ...
              description: "Using OpenAI Whisper for better Thai language support",
            });
            setIsListening(false);
            setTimeout(() => {
            ...
            toast({
              ...
              description: event.error === 'no-speech' ? 'No speech detected. Please try again.' : 'Unable to process speech. Please try typing instead.',
              variant: "destructive"
            });
          }
        };
  ...
  const analyzeSentiment = (text: string): 'positive' | 'negative' | 'neutral' | 'excited' | 'concerned' => {
    const positiveWords = ['great', 'excellent', 'amazing', 'love', 'perfect', 'fantastic', 'awesome', 'brilliant', 'good', 'nice', 'happy', 'excited', 'bullish', 'moon', 'pump'];
    const negativeWords = ['bad', 'terrible', 'awful', 'hate', 'horrible', 'worst', 'disgusting', 'sad', 'worried', 'concerned', 'bearish', 'dump', 'crash', 'loss', 'risk'];
    const excitedWords = ['wow', 'amazing', 'incredible', 'to the moon', '🚀', 'pump', 'moon', 'lambo', 'diamond hands'];
    const concernedWords = ['worried', 'concerned', 'scared', 'risk', 'loss', 'crash', 'dump', 'bear market', 'recession'];
  ...
  const detectIntent = (text: string): string => {
    ...
    if (lowerText.includes('what') || lowerText.includes('how') || lowerText.includes('why') || lowerText.includes('explain') ||
        lowerText.includes('อะไร') || lowerText.includes('ยังไง') || lowerText.includes('ทำไม') || lowerText.includes('อธิบาย')) {
      return 'informational';
    }
    if (lowerText.includes('should i') || lowerText.includes('recommend') || lowerText.includes('suggest') || lowerText.includes('advice') ||
        lowerText.includes('ควร') || lowerText.includes('แนะนำ') || lowerText.includes('คำแนะนำ') || lowerText.includes('ช่วย')) {
      return 'actionable';
    }
    if (lowerText.includes('price') || lowerText.includes('market') || lowerText.includes('analysis') ||
        lowerText.includes('ราคา') || lowerText.includes('ตลาด') || lowerText.includes('วิเคราะห์')) {
      return 'market_analysis';
    }
    if (lowerText.includes('portfolio') || lowerText.includes('allocation') || lowerText.includes('rebalance') ||
        lowerText.includes('ปรับ') || lowerText.includes('จัดสรร') || lowerText.includes('เปลี่ยน') || 
        lowerText.includes('แก้') || lowerText.includes('optimize') || lowerText.includes('โปรฟ') ||
        lowerText.includes('ลงทุน') || lowerText.includes('ปอร์ต')) {
      return 'portfolio_management';
    }
    if (lowerText.includes('hi') || lowerText.includes('hello') || lowerText.includes('hey') ||
        lowerText.includes('สวัสดี') || lowerText.includes('หวัดดี') || lowerText.includes('ฮาย')) {
      return 'greeting';
    }
  ...
  const detectPortfolioAdjustmentRequest = (text: string): boolean => {
    ...
    const englishKeywords = [
      'adjust portfolio', 'rebalance', 'change allocation', 'optimize portfolio',
      'portfolio suggestion', 'investment advice', 'adjust allocation', 'modify portfolio',
      'portfolio recommendation', 'investment recommendation', 'should i change',
      'how to adjust', 'optimize investment', 'portfolio strategy'
    ];
    
    const thaiKeywords = [
      'ปรับพอร์ต', 'ปรับโปรตโฟลิโอ', 'เปลี่ยนการลงทุน', 'จัดสรรใหม่',
      'แนะนำการลงทุน', 'คำแนะนำพอร์ต', 'ปรับการจัดสรร', 'แก้ไขพอร์ต',
      'แนะนำโปรตโฟลิโอ', 'ควรปรับ', 'ช่วยปรับ', 'ให้คำแนะนำ',
      'วิธีปรับ', 'กลยุทธ์การลงทุน', 'ปรับสัดส่วน', 'เปลี่ยนสัดส่วน'
    ];
  ...
  const generatePersonalizedResponse = (userMessage: string, sentiment: string, intent: string, profile: UserProfile | null): string => {
    ...
    switch (communicationStyle) {
      case 'formal':
        ...
        break;
      case 'technical':
        responsePrefix = 'According to technical indicators and on-chain metrics, ';
        break;
      default:
        responsePrefix = sentiment === 'excited' ? '🚀 I can see you\'re excited! ' : 
                        sentiment === 'concerned' ? '😌 I understand your concerns. ' : '';
    }
    
    // Adjust based on risk tolerance
    switch (riskTolerance) {
      case 'conservative':
        tonalAdjustment = ' I recommend focusing on stable, well-established assets with lower volatility.';
        break;
      case 'aggressive':
        tonalAdjustment = ' Given your risk appetite, you might consider more growth-oriented opportunities.';
        break;
      default:
        tonalAdjustment = ' Let\'s find a balanced approach that matches your investment strategy.';
    }
  ...
  const updateConversationContext = (userMessage: string, intent: string) => {
    setConversationContext(prev => {
      ...
      else if (intent === 'portfolio_management') newContext.currentTopic = 'portfolio';
      else if (intent === 'informational') newContext.currentTopic = 'learning';
      
      // Add to topic history if it's a new topic
      if (newContext.currentTopic !== prev.currentTopic && newContext.currentTopic) {
        newContext.topicHistory = [...prev.topicHistory.slice(-4), newContext.currentTopic];
      }
    ...
    setMessages([
      {
        id: '1',
        sender: 'ai',
        content: 'Hello! I\'m your MonadAI assistant. I can help you manage your portfolio, provide market insights, and suggest optimal allocations. How can I assist you today?',
        timestamp: new Date(),
      }
    ]);
  };
  ...
  const triggerAIInsight = (insight: any) => {
    ...
    const personalizedInsight = userProfile ? 
      getPersonalizedInsight(adaptedInsight, userProfile) : adaptedInsight;
    ...
    setTimeout(() => {
      ...
      const formattedContent = personalizedInsight.action?.changes ? 
        formatPortfolioSuggestion(personalizedInsight.action, personalizedInsight.content) : 
      ...
      setTimeout(() => {
        const followUpQuestion = generateFollowUpQuestion(personalizedInsight, userProfile);
        if (followUpQuestion) {
  ...
  const generateFollowUpQuestion = (insight: any, profile: UserProfile | null): string | null => {
    const questions = [
  ...
  const handleSendMessage = async () => {
    ...
    if (userProfile) {
      const updatedProfile = {
        ...
        favoriteTopics: [...new Set([...userProfile.favoriteTopics, intent])].slice(-10) // Keep last 10 topics
      };
    ...
    const userMessage: ChatMessage = {
      ...
      context: {
        ...
        questionType: intent === 'informational' ? 'informational' : 
                     intent === 'actionable' ? 'actionable' : 'exploratory',
        userMood: sentiment
      }
    };
    ...
    if (isGeminiEnabled) {
      try {
        ...
        const personalizedPrefix = generatePersonalizedResponse(userMessage.content, sentiment, intent, userProfile);
        
        // Generate response using Gemini
        const { content, action } = await generateChatResponse(userMessage.content, recentMessages);
        ...
        const isPortfolioRequest = detectPortfolioAdjustmentRequest(userMessage.content);
    ...
    setTimeout(() => {
      ...
      if (isPortfolioRequest) {
        const adjustmentText = speechLanguage === 'th-TH' ? 
        ...
        ruleBasedResponse = {
          ...
          action: {
            type: 'rebalance',
            description: speechLanguage === 'th-TH' ? 'ปรับพอร์ตโฟลิโอ' : 'Adjust Portfolio',
            changes: []
          }
        };
      }
                    ...
                    <div className="flex items-center mb-2">
                      ...
                      <div className="ml-2 flex flex-col">
                        <span className="font-medium">
                          {msg.sender === 'user' ? 'You' : 'MonadAI Assistant'}
                        </span>
                        {msg.sender === 'ai' && msg.sentiment && (
                          <span className="text-xs text-muted-foreground">
                            {msg.sentiment === 'positive' ? '😊 Optimistic' : 
                             msg.sentiment === 'concerned' ? '🤔 Analytical' : 
                             msg.sentiment === 'excited' ? '🚀 Excited' : '🤖 Neutral'}
                          </span>
                        )}
                      </div>
                      <div className="ml-auto flex flex-col items-end">
                        ...
                        {msg.action?.confidence && (
                          <span className="text-xs text-yellow-400">
                            {Math.round(msg.action.confidence * 100)}% confidence
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {msg.action && (
                      ...
                      >
                        ...
                        {msg.action.description || 
                         (msg.action.type === 'rebalance' && speechLanguage === 'th-TH' ? 'ปรับพอร์ตโฟลิโอ' : 'Adjust Portfolio') ||
                         (msg.action.type === 'analysis' && speechLanguage === 'th-TH' ? 'วิเคราะห์ตลาด' : 'Market Analysis') ||
                         (msg.action.type === 'trade' && speechLanguage === 'th-TH' ? 'ข้อเสนอการเทรด' : 'Trading Suggestion') ||
                         (msg.action.type === 'protection' && speechLanguage === 'th-TH' ? 'ป้องกันความเสี่ยง' : 'Risk Protection') ||
                         (speechLanguage === 'th-TH' ? 'ดำเนินการ' : 'Take Action')}
                      </Button>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-start">
                  <div className="bg-cosmic-700 rounded-2xl p-4 max-w-[80%] border border-yellow-500/30">
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 font-medium">MonadAI Assistant</span>
                    </div>
        ...
        <CardFooter className="p-4 border-t border-[#ffffff1a]">
          ...
          {conversationContext.currentTopic && (
            <div className="mb-2 text-xs text-muted-foreground">
              💬 Currently discussing: <span className="text-yellow-400">{conversationContext.currentTopic}</span>
              {userProfile && (
                <span className="ml-4">
                  👤 {userProfile.communicationStyle} mode | 📊 {userProfile.riskTolerance} risk
                </span>
              )}
            </div>
          )}
          ...
          {isListening && (
            <div className="mb-2 flex items-center space-x-2 text-sm">
              <div className="flex items-center text-yellow-400">
                ...
                <span>
                  {useWhisperFallback ? 'AI Enhanced' : 'Browser'} Recognition - {speechLanguage === 'en-US' ? 'English' : 'Thai'}
                </span>
              </div>
              {interimTranscript && (
                <span className="text-muted-foreground italic">
                  "{interimTranscript.length > 50 ? interimTranscript.substring(0, 50) + '...' : interimTranscript}"
                </span>
              )}
            </div>
          )}
          ...
          <div className="mb-2 flex items-center justify-between">
            ...
            {!speechSupported && (
              <div className="text-xs text-red-400">🚫 Speech not supported</div>
            )}
          </div>
          ...
          <div className="flex w-full space-x-2">
              ...
              placeholder={
                ...
                conversationContext.currentTopic 
                  ...
                  : speechLanguage === 'th-TH' 
                    ...
                    : "Ask MonadAI assistant about market trends, tokens, or portfolio advice..."
              }
              value={message + (interimTranscript && !isListening ? ` ${interimTranscript}` : '')}
            ...
            {speechSupported && (
              <Button
                ...
                title={isListening ? "Stop listening" : `Start listening in ${speechLanguage === 'th-TH' ? 'Thai' : 'English'}`}
              >
